local _, psy = ...
if not psy.Rotations.HUNTER then
    psy.Rotations.HUNTER = {}
end
local Hunter = psy.Rotations.HUNTER
local Player, Buff, Debuff, Spell, Target, Pet, Trait, GCD, Unit
...
local function SingleTarget()
    ...
    if (Player.Power - Spell.CobraShot.Cost + Player.PowerRegen * (Spell.KillCommand:CurrentCD() - 1) > Spell.KillCommand.Cost or Spell.KillCommand:CurrentCD() > (1 + GCD)) and Spell.KillCommand:CurrentCD() > 1 then
...
function Hunter.BeastMastery()
    Locals()
    if Target and Target:IsAttackable() then
        p.RunMacroText("/startattack")
        SingleTarget()
    end
end